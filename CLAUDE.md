...
- Status: 디자인 시안 전체를 React로 데모 구현 완료 (목데이터) + 통계 카드 클릭 → **상세 페이지**(기간 년/월/일/시 지정, 항목별 통계·그래프·이벤트 목록) 11종 구현(조류 활동 포함). 다음 단계는 Rust 백엔드(폴더 감시·파서·DB) 연결.
...
### 화면
- 사이드바(공항 목록 + 분석/지도/설정 내비 + 폴더 감시 상태). `분석`(view 키는 여전히 'stats') 아래에 상세 페이지 11종이 중첩 메뉴(`.nav__sub`, `PANEL_KEYS` 순서)로 펼쳐짐 — stats 뷰일 때만 표시, 클릭 → `openDetail(key)`. 상세가 열려 있으면 하위 항목이 선택 강조(primary), 부모는 `.nav__item--open` 은은한 톤
...
- 분석 뷰(`StatsView`): KPI 4장 + 카드 13종 (온도/노점, 바람, 측풍/배풍, 활주로 비율, 전환 이벤트, 히트맵, 갱신 빈도, 구름/접근, QNH, 시정, 기상 태그, 조류 활동). **모든 카드는 `.card--link`로 클릭 → 상세 페이지** (KPI: 총 수신/발행 간격→update, 최다 활주로→runway, 평균 QNH→qnh). 카드 안의 클릭 요소(타임라인 도트·히트맵 셀·갱신 막대)는 stopPropagation
...
- 지도 뷰: Leaflet + 타임 스크러버(재생 350ms, 레코드가 240건 넘으면 균등 솎아냄) + 플로팅 요약 카드. 조류 활동 섹터(부채꼴)와 BIRD ACTIVITY 카드는 **선택 시각 전문의 `birds`**를 그림(보고 없으면 '보고 없음'); 충돌/회피 보고 마커는 아직 정적 데모
- 설정 뷰: 감시 폴더 / 최근 가져온 파일
- 원문 모달: ←/→ 이전·다음, ESC 닫기. 탐색 목록은 열어준 쪽의 배열(현재 기간/히트맵 7일/상세 기간)
- 조류 데이터 모델: `AtisRecord.birds: BirdReport[]` ({kind HVY|LGT, dir 8방위, nm}) — 전문 remarks의 `CTN BIRD ACTIVITY HVY FLOCK 5NM NW OF AD` 토큰에 대응. 표시 헬퍼(`BIRD_COLOR`, `BIRD_KIND_LABEL`, `birdHead`)와 카드 파생값 `computeBirdCard`는 `data/stats.ts`, 상세는 `data/detail/bird.ts`. CSV에 `birds` 열 포함
...
src/
  ...
  data/
    types.ts                   AtisRecord(ts·birds 포함)·BirdReport·DetailKey·TimeWindow·HeatRow 등 도메인 타입
    ...
    stats.ts                   computeStats(recs, range, xwLimit): 카드 파생값 · computeHeatRows(recs, from, to) · TAG_DESC · windColor · BIRD_COLOR/birdHead/computeBirdCard
  ...
  components/
    ...
    detail/panels/<Key>Panel.tsx  temp/wind/xwind/runway/heat/update/cloud/qnh/vis/tags/bird 11종 (각각 data/detail/<key>.ts와 짝)
...
- 상세 패널 정의 기준(카드와 동일): 시정 저하 vis<10, 안개 위험 스프레드 ≤2°C, 강풍 ≥15KT·정온 ≤3KT, 측풍 한계 `XW_LIMIT`(15KT)·배풍 기준 5KT, 저실링 <1000FT, QNH 급변 |Δ| ≥ 3hPa/3h, 정기 발행 = 분 00/30(그 외 임시 갱신), 활주로 전환 = 직전 전문과 rwy 다름, 조류 보고 전문 = birds 1건 이상·무리 건수 = 전문 × 무리·활동 구간 = 연속 보고 병합(공백 3h 초과 시 분리)·새벽 20–23Z/저녁 07–10Z 집중 패턴 ≥60%.
- 목데이터 의미(`mock.ts`): 정기 발행 :00/:30 + 상태 급변 시 :15/:45 임시 갱신(조류 보고 변화 포함), 계절(기온·풍향 NW↔SW·QNH·강수확률)·일변화, 안개 03–06Z, 태그 BR/FG/HZ/RA/SN/TS, 활주로는 느린 풍향 250°~70° → 32. 조류 활동(`birdsAt`)은 일 단위 시드 — 봄(4월 초)·가을(10월 말) 철새 이동기와 겨울에 잦고 새벽 20.5–23Z·저녁 7.5–11Z 집중, 방위는 NW/N/W 가중, 거리 1–6NM, 30%는 두 번째 작은 무리. **디자인 데모용으로 `MOCK_NOW` 당일(08-15)은 시안과 같은 조류 보고 3건(HVY 5NM NW · LGT 3NM NE · LGT 2NM SE, `DEMO_BIRDS`)을 종일 고정** — 지도 기본 화면에 섹터·카드가 채워지게 함(백엔드 연결 시 제거). `MOCK_NOW` = 2026-08-15 12:00Z(데모라 날짜 갱신하지 않음), `DATA_START` = 2024-01-01.
- 개발/스크린샷 편의: URL 해시로 초기 상태 지정 가능 — `#view=map&range=7d&raw=3&idx=22&aerial=0&zoom=15` (항공사진은 기본 ON, `aerial=0`으로 끔; `idx`는 타임 스크러버 위치), 상세는 `#view=stats&detail=bird&from=2026040100&to=2026043000` (YYYYMMDDHH UTC).
